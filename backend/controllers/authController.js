...
export const register = async (req, res) => {
  try {
    ...
    if (existing.length > 0) return res.status(400).json({ message: 'Email ekziston tashmë' });
...
export const login = async (req, res) => {
  try {
    ...
    if (users.length === 0) return res.status(400).json({ message: 'Email ose password i gabuar' });

    const user = users[0];
    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return res.status(400).json({ message: 'Email ose password i gabuar' });
...
export const refresh = async (req, res) => {
  try {
    ...
    if (tokens.length === 0) return res.status(403).json({ message: 'Token i pavlefshëm' });

    const decoded = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET);

    // Fshi te vjetrin — rotacion
